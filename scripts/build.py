...
def main():
    ...
    parser.add_argument("-c", help="config (Debug or Release)", default="Release", dest="config")
    ...
    if args.run_tests:
        rc = subprocess.call("ctest . --output-on-failure -c {}".format(args.config).split(), cwd=os.path.join(src_dir,args.out_dir))
        if rc != 0:
            sys.exit(1)
