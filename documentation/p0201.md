...
* Preconditions: `C` and `D` meet the `Cpp17CopyConstructible` 
  ...
  The initialization of objects of type `C` and `D` does not exit 
  via an exception.
...
* Remarks: This constructor can allow an implementation to avoid the need for
...
[Note 2: move construction of an owned object may be used by an implementation to
avoid the need for use of dynamic memory. - end note]
...
* Effects: Exchanges the states of `p` and `*this`.
...
[Note 3: Implementations are encouraged to avoid multiple allocations. - end note]
