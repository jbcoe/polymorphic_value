...
Using `polymorphic` a copyable composite object with polymorphic
...
A nullable `polymorphic<T>` could be mimicked with `std::optional<polymorphic<T>>`
...
This is somewhat cumbersome so `make_polymorphic` has been modified to
...
`polymorphic<T>` has been renamed few times, more recently to `polymorphic_value<T>`
as its behaviour changed to represent a value type. We've responded to the often offered
criticism that the `_value` suffix was unnecessary and not present in `std::optional`.
While `std::polymorphic` feels a little odd to the authors, we'd do well to remember that there
will always be a template argument `T` and `polymorphic<Shape>` has a rather nice feel to it.
...
The template parameter `T` of `polymorphic<T>` shall be a non-union class
...
### X.Z.4 Class template `polymorphic` destructor [polymorphic.dtor]
