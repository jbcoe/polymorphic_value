...
template <class T, class A = std::allocator<T>, class... Ts>
  polymorphic_value<T> allocate_polymorphic_value(std::allocator_arg_t, A& a, Ts&&... ts);
...
polymorphic_value<T> allocate_polymorphic_value(allocator_arg_t, A& a, Ts&&... ts);
...
```cpp
template <class T, class U = T, class A = allocator<U>, class... Ts>
polymorphic_value<T> allocate_polymorphic_value(const A& a, Ts&&... ts);
```
